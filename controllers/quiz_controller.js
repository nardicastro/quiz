...
exports.index = function(req,res,next)
{
  if(req.query.search) {
    ...
    {
      res.render('quizes/index.ejs',{ quizes: quizes,filtro: req.query.search, errors: []});
    }).catch(function(error) { next(error); });
  }
  else {
    models.Quiz.findAll()
    		.then(
    			function(quizes) {
    					res.render('quizes/index', {quizes: quizes, filtro:'*', errors: []});
    			}
    		).catch(function(error) { next(error);})
  }
};
...
exports.create = function(req,res){
  ...
  quiz.validate().then(function(err){
    if (err) {
      ...
      quiz // save: guarda eb BBDD campos pregunta y respuesta de quiz
...
exports.update = function(req,res){
  ...
  then(function(err){
    if (err) {
      ...
      req.quiz // save: guarda eb BBDD campos pregunta y respuesta de quiz
      .save({fields: ["pregunta", "respuesta"]})
      .then(function(){ res.redirect('/quizes');});
      // Redirección HTTP (URL relativo) lista de preguntas
      }
